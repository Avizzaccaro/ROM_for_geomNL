...
for iChunk=1:pts
    ...
    for iPoint = 1:nb_pt
        ...
        for i=1:L
            ...
            Y_dof=Y_dof+sys.parameters.Phi(dof,i)*S(:,i)';
            %
            for j=1:L
                X_dof=X_dof+sys.parameters.a(dof,i,j)*(R(:,i).*R(:,j))'+...
                    +sys.parameters.b(dof,i,j)*(S(:,i).*S(:,j))'...
                    +0*sys.parameters.cd(dof,i,j)*(R(:,i).*S(:,j))';
                Y_dof=Y_dof++sys.parameters.ad(dof,i,j)*(R(:,i).*R(:,j))'+...
                    +0*sys.parameters.bd(dof,i,j)*(S(:,i).*S(:,j))'...
                    +0*sys.parameters.c(dof,i,j)*(R(:,i).*S(:,j))';
            end
        end
...
plot(omega_nl,ampl,'Color',[0,0,.6],'LineWidth',2);
xlabel('\omega [rad/s]')
ylabel('A [m]')
%
legend('DNF SO')
