function [Rtot,dRaux,Forcing] = QM_MD_equations(sys,t,Utot,dUtot,~)
...
R(2*sys.nz/3+1:sys.nz)= - stiffness*x - lambda*damping*y -  mass*z;
for p=1:n_modes
for i=1:n_modes
    for j=1:n_modes
        ...
        for k=1:n_modes
            R(2*sys.nz/3+p) = R(2*sys.nz/3+p)...
                ...
                - 0*HVd(p,i,j,k)*y(i)*r(pick(j,k))...		%	HV, cubic term in velocity (damp)
...
for kk=2*sys.nz/3+1:sys.nz
    Forcing(:,kk) = 0*force(kk-2*sys.nz/3)*cos(t);
end
